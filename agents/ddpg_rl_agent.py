...
#        return action
    ...
    def make_noisy(self, action):
#        brakemu = 0.1
#        if 60000 < self.model.run_inferences() < 100000:
#            brakemu = max(0.1, min(0.5, (self.model.run_inferences()-60000)/40000))
        def Ornstein(x,mu,theta,sigma):
            return theta * (mu - x) + sigma * np.random.randn(1)
        action[0] += max(self.epsilon*4, 0) * Ornstein(action[0],  0.5 , 0.85, 0.10)
        action[1] += max(self.epsilon*4, 0) * Ornstein(action[1],  0.0 , 0.85, 0.05)  
