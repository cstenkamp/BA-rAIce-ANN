...
pre_train_steps = 90000 #How many steps of random actions before training begins.
...
class Qnetwork():
    def __init__(self,h_size,num_actions):
        ...
        self.ff_inputs = tf.placeholder(tf.float32, shape=[None, 1], name="ff_inputs")
        ...
        for episode in range(num_episodes):
            ...
            for j in range(1000): #max.ep-step
                ...
                if np.random.rand(1) < e or total_steps < pre_train_steps:
                    ...
                    a = sess.run(mainQN.predict,feed_dict={mainQN.ff_inputs:[[s]]})[0]
                ...
                if total_steps > pre_train_steps:
                    ...
                    if total_steps % (update_freq) == 0:
                        ...
                        Q1 = sess.run(mainQN.predict,feed_dict={mainQN.ff_inputs:trainBatch[3]})
                        Q2 = sess.run(targetQN.Qout,feed_dict={targetQN.ff_inputs:trainBatch[3]})
                        end_multiplier = -(trainBatch[4] - 1)
                        doubleQ = Q2[range(MINIBATCH_SIZE),Q1]
                        targetQ = trainBatch[2] + (y*doubleQ * end_multiplier)
                        #Update the network with our target values.
                        _ = sess.run(mainQN.updateModel, \
                            feed_dict={mainQN.ff_inputs:trainBatch[0],mainQN.targetQ:targetQ, mainQN.actions:trainBatch[1]})
